...
class ModelReportProduct extends Model {
	...
	public function getDepartments($product_id, $data = array()) {
		if (empty($data['filter_date_start']) || empty($data['filter_date_end'])) return array();

		$query = $this->db->query("SELECT *, SUM(d.quantity) AS total FROM " . DB_PREFIX . "delivery d, " . DB_PREFIX . "department dp, " . DB_PREFIX . "department_description dpd WHERE d.department_id = dp.department_id AND product_id = '" . (int)$product_id . "' AND DATE(d.delivery_date) >= '" . $data['filter_date_start'] . "' AND DATE(d.delivery_date) <= '" . $data['filter_date_end'] . "' AND d.department_id = dpd.department_id GROUP BY d.department_id");
