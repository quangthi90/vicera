...
class ModelCatalogProduct extends Model {
	public function addProduct($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', price = '" . (float)$data['price'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW(), department_id = '" . (int)$user['department_id'] . "'");

		$product_id = $this->db->getLastId();

		$receipt_quantity_total = $this->db->query("SELECT SUM(quantity) AS total FROM " . DB_PREFIX . "receipt WHERE product_id = '" . (int)$product_id . "'")->row['total'];
		if ($receipt_quantity_total == null) $receipt_quantity_total = 0;

		$delivery_quantity_total = $this->db->query("SELECT SUM(quantity) AS total FROM " . DB_PREFIX . "delivery WHERE product_id = '" . (int)$product_id . "'")->row['total'];
		if ($delivery_quantity_total == null) $delivery_quantity_total = 0;

		$quantity = $receipt_quantity_total - $delivery_quantity_total;
	...
	public function editProduct($product_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $this->db->escape($data['model']) . "', manufacturer_id = '" . (int)$data['manufacturer_id'] . "', price = '" . (float)$data['price'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getProduct($product_id) {
		...
		if ($this->user->getGroupId() != 1) {
			...
			$sql .= " AND p.department_id = '" . $user['department_id'] . "'";
		}
