...
class ControllerDepartmentDepartment extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('department/department', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('department/department/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['delete'] = $this->url->link('department/department/delete', 'token=' . $this->session->data['token'] . $url, 'SSL');
		$data['repair'] = $this->url->link('department/department/repair', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		foreach ($results as $result) {
			$data['departments'][] = array(
				...
				'edit'        => $this->url->link('department/department/edit', 'token=' . $this->session->data['token'] . '&department_id=' . $result['department_id'] . $url, 'SSL'),
				'delete'      => $this->url->link('department/department/delete', 'token=' . $this->session->data['token'] . '&department_id=' . $result['department_id'] . $url, 'SSL')
			);
		}
		...
		$data['sort_name'] = $this->url->link('department/department', 'token=' . $this->session->data['token'] . '&sort=name' . $url, 'SSL');
		$data['sort_sort_order'] = $this->url->link('department/department', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, 'SSL');
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['department_id'])) {
			$data['action'] = $this->url->link('department/department/add', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['action'] = $this->url->link('department/department/edit', 'token=' . $this->session->data['token'] . '&department_id=' . $this->request->get['department_id'] . $url, 'SSL');
		}

		$data['cancel'] = $this->url->link('department/department', 'token=' . $this->session->data['token'] . $url, 'SSL');
		...
		if (isset($this->request->post['department_description'])) {
			...
			$data['department_description'] = $this->model_department_department->getDepartmentDescriptions($this->request->get['department_id']);
		} else {
		...
		if (isset($this->request->post['sort_order'])) {
			...
			$data['sort_order'] = $department_info['sort_order'];
		} else {
			$data['sort_order'] = 0;
		}
	...
	protected function validateForm() {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			$this->load->model('department/url_alias');

			$url_alias_info = $this->model_department_url_alias->getUrlAlias($this->request->post['keyword']);
		
			if ($url_alias_info && isset($this->request->get['department_id']) && $url_alias_info['query'] != 'department_id=' . $this->request->get['department_id']) {
				$this->error['keyword'] = sprintf($this->language->get('error_keyword'));
			}

			if ($url_alias_info && !isset($this->request->get['department_id'])) {
				$this->error['keyword'] = sprintf($this->language->get('error_keyword'));
			}
