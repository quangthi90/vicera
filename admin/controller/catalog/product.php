...
class ControllerCatalogProduct extends Controller {
	...
	public function copy() {
		...
		$data['text_form'] = $this->language->get('button_copy');
		...
		$data['action'] = $this->url->link('catalog/product/exportOrder', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('catalog/product_copy.tpl', $data));
	}
	...
	protected function getForm() {
		$this->document->addScript('view/javascript/jquery.number.js');
	...
	public function exportOrder() {
		...
		foreach ($products as $product) {
			if (!empty($test_key_product[$product['id']])) continue;
			$test_key_product[$product['id']] = $product['id'];

			$quantity = $product['quantity'];
			$result = $this->model_catalog_product->getProduct($product['id']);
			...
			try {
				...
				// Số lượng
				$arr_data['D' . $row] = $quantity;
				// Đơn giá
				$arr_data['E' . $row] = $result['price'];
				// Thành tiền
				$arr_data['F' . $row] = $result['price'] * $quantity;
			} catch (Exception $e) {
